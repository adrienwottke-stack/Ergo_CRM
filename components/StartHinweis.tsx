...
export default function StartHinweis({ phase, kompakt = false }: { phase:string; kompakt?: boolean }) {
  const router=useRouter(); const [pending,startTransition]=useTransition(); const [error,setError]=useState(false);
  const text=phase === "INTRO" ? ["Deinen Start fortsetzen", "Wir machen dort weiter, wo du aufgehört hast.", "Start fortsetzen"]
  ...
  return <section aria-label="Dein nächster Startschritt" className={kompakt ? "space-y-3" : `${card} space-y-3 p-5 sm:p-6`}>
    ...
    <button disabled={pending} className={kompakt ? "crm-primary-action w-full disabled:opacity-60" : btnPrimary} onClick={()=>startTransition(async()=>{setError(false);try {router.push(await startFortsetzen());router.refresh();} catch {setError(true);}})}>{pending ? "Stand laden …" : text[2]}</button>
