...
import { naechsteFkOberhalb } from "@/lib/ausbau";
 ...
 * Autorisierung ist enger als sonst in dieser Datei: nicht der ganze Ast,
 * sondern nur die naechste einloggbare Fuehrungskraft (siehe unten).
 *
...
export async function ausbauFreischalten(formData: FormData) {
  ...
  // BEWUSST NICHT inMeinerStruktur(). Die deckt den ganzen Ast ab, und damit
  // duerfte auch ein Vorgesetzter zwei Ebenen hoeher freischalten - an der
  // direkten Fuehrungskraft vorbei. Plan und ADR-0005 sagen: die NAECHSTE
  // einloggbare Fuehrungskraft oberhalb entscheidet, weil sie den Menschen
  // kennt. Der Admin darf trotzdem, er ist der Notausgang der Mechanik
  // (dieselbe Ausnahme wie in inMeinerStruktur).
  const mitglied = await prisma.user.findUnique({
    where: { id: memberId },
    select: { id: true, path: true },
  });
  if (!mitglied || mitglied.id === user.id) return;

  if (user.role !== "ADMIN") {
    const zustaendig = await naechsteFkOberhalb(mitglied.path, mitglied.id);
    if (zustaendig?.id !== user.id) return;
  }
