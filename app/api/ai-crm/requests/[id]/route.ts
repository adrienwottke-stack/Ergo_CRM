...
import { cancelActionPlans, executeActionPlan } from "@/lib/ai-crm/action-plans";
...
const schema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("confirm"), actionIds: z.array(z.string().min(1)).min(1).max(12) }).strict(),
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    if (parsed.data.action === "cancel") await cancelActionPlans(prisma, user.id, current.id);
    else {
      ...
      await executeActionPlan(prisma, { userId: user.id, requestId: current.id, actionIds: parsed.data.actionIds });
    }
